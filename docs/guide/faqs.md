...
本文整理了一些常见问题，如有其他疑问可即时反馈。

## 一、常见问题 {#common-questions}

* **Q**: ContextPath 的配置以及使用场景？ 

    **A**: 有时负载LB需要通过ContextPath来区分不同的服务，这时就需要配置应用的ContextPath。

    1. 为自己的 Spring boot 应用配置ContextPath
    ```yaml
      server:
        port: 8081
  
        servlet:
        context-path: /revenueapi # 为Spring boot 应用配置ContextPath
    
    ```
    2. 修改console中 ui 公众中对应的 .env 文件。

    ```.yaml
      PB_BACKEND_URL = "/revenueapi" # 与 Spring 应用中的 context-path 保持一直
    ```
