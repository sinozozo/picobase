...
public class PbLogic {
     ...
     */
    public PbLogic(String loginType) {
     ...
     * @return 对象自身
     */
    public PbLogic setLoginType(String loginType) {

        // 先清除此 PbLogic 在全局 PbManager 中的记录
        if (PbInnerUtil.isNotEmpty(this.loginType)) {
            PbManager.removePbLogic(this.loginType);
        }

        // 赋值
        this.loginType = loginType;

        // 将新的 loginType -> PbLogic 映射关系 put 到 PbManager 全局集合中，以便后续根据 LoginType 进行查找此对象
        PbManager.putPbLogic(this);
    ...
    /**
     * 写入当前 PbLogic 单独使用的配置对象
     *
     * @param config 配置对象
     * @return 对象自身
     */
    public PbLogic setConfig(PbConfig config) {
        this.config = config;
        return this;
    }

    /**
     * 返回当前 PbLogic 使用的配置对象，如果当前 PbLogic 没有配置，则返回 null
     *
     * @return /
     */
    ...
    /**
     * 返回当前 PbLogic 使用的配置对象，如果当前 PbLogic 没有配置，则返回全局配置对象
     *
     * @return /
     */
    ...
    public void setTokenValue(String tokenValue, PbLoginModel loginModel) {
        ...
        if (PbInnerUtil.isEmpty(tokenValue)) {
            return;
        }
    ...
    public void setTokenValueToStorage(String tokenValue) {
        ...
        if (PbInnerUtil.isEmpty(tokenPrefix)) {
    ...
    public String getTokenValue(boolean noPrefixThrowException) {
        ...
        String tokenPrefix = getConfigOrGlobal().getTokenPrefix();
        if (PbInnerUtil.isNotEmpty(tokenPrefix)) {
    ...
    protected String distUsableToken(Object id, PbLoginModel loginModel) {
        ...
        if (PbInnerUtil.isNotEmpty(loginModel.getToken())) {
        ...
        if (isConcurrent) {
            ...
            if (getConfigOfIsShare()) {
                ...
                // 如果有值，那就直接复用
                if (PbInnerUtil.isNotEmpty(tokenValue)) {
                    return tokenValue;
                }
    ...
    public Object getLoginId() {
        ...
        String loginId = getLoginIdNotHandle(tokenValue);
        if (PbInnerUtil.isEmpty(loginId)) {
    ...
    public <T> T getLoginId(T defaultValue) {
        ...
        return (T) PbInnerUtil.getValueByType(loginId, defaultValue.getClass());
    }
    ...
    public boolean isValidLoginId(Object loginId) {
        ...
        return PbInnerUtil.isNotEmpty(loginId) && !NotLoginException.ABNORMAL_LIST.contains(loginId.toString());
    }
    ...
    public void updateTokenToIdMapping(String tokenValue, Object loginId) {
        // 先判断一下，是否传入了空值
        PbException.throwBy(PbInnerUtil.isEmpty(loginId), "loginId 不能为空", PbErrorCode.CODE_11003);
    ...
    public PbSession getSessionBySessionId(String sessionId, boolean isCreate, Consumer<PbSession> appendOperation) {
        ...
        if (PbInnerUtil.isEmpty(sessionId)) {
            return null;
        }
    ...
    public void checkByAnnotation(SaCheckPermission at) {
        ...
        try {
            ...
            for (String role : at.orRole()) {
                String[] rArr = PbInnerUtil.convertStringToArray(role);
    ...
    public void disableLevel(Object loginId, String service, int level, long time) {
        ...
        if (PbInnerUtil.isEmpty(service)) {
    ...
    public void checkDisableLevel(Object loginId, String service, int level) {
        ...
        // 2、再判断被封禁的等级是否达到了指定级别
        Integer disableLevel = PbInnerUtil.getValueByType(value, int.class);
    ...
    public int getDisableLevel(Object loginId, String service) {
        ...
        // 2、转为 int 类型返回
        return PbInnerUtil.getValueByType(value, int.class);
    }
    ...
    public boolean isSafe(String tokenValue, String service) {
        ...
        return !(PbInnerUtil.isEmpty(value));
    }
